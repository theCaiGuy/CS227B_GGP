 ...
 */
public final class MGJTimeLimitedMobilityGamerBFS extends SampleGamer
{
	/*
	...
	private Move bestMove(Role role, MachineState state, List<Move> actions, long start, long timeout) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		Queue<Stack<Move>> possible_actions = new LinkedList<Stack<Move>>();
		for (Move action : actions) {
			Stack<Move> new_stack = new Stack<Move>();
			new_stack.push(action);
			possible_actions.add(new_stack);
		}
//		Stack<Move> best_stack = minScore(role, state, start, timeout, possible_actions);
//		double score = 0;
...
//			}
//		}
		return possible_actions.element().peek();
	}

	private Stack<Move> minScore(Role role, MachineState state, long start, long timeout, Queue<Stack<Move>> possible_actions) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		Queue<Stack<Move>> next_actions = new LinkedList<Stack<Move>>();
		while (!possible_actions.isEmpty()) {
			Stack<Move> curr_stack = possible_actions.peek();
			Move curr_move = curr_stack.peek();
			List<List<Move>> allJointActions = getStateMachine().getLegalJointMoves(state, role, curr_move);
		}
