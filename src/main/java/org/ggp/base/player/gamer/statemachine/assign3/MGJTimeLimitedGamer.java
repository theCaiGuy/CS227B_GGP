 ...
 */
public final class MGJTimeLimitedGamer extends SampleGamer
{
	/*
	...
	public Move stateMachineSelectMove(long timeout) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
		...
		Move selection = bestMove(role, currentState, moves, role_index, start);
	 ...
	 */
	private Move bestMove(Role role, MachineState state, List<Move> actions, int role_index, long start) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		Move chosenMove = actions.get(0);
		int score = 0;
		// loop through all actions and find the best score and return this
		for (int i = 0; i < actions.size(); i++) {
			int result = minScore(role, actions.get(i), state, role_index, start);
			if (result > score) {
				score = result;
	 ...
	 */
	private int minScore(Role role, Move move, MachineState state, int role_index, long start) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		int score = 100;
		...
		for (int i = 0; i < allJointActions.size(); i++) {
			...
			int result = maxScore(role, updatedState, role_index, start);
			if (result < score) {
				score = result;
			}
		}
	 ...
	 */
	private int maxScore(Role role, MachineState state, int role_index, long start) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
		...
		} else if ((System.currentTimeMillis() - start) >= limit) return 0;
		else {
